...
import type { Question, QuestionInput, QuestionFilters, QuestionProgress, QuestionType, Track } from '@/types/question'
...
export async function getQuestions(opts: GetQuestionsOptions = {}): Promise<GetQuestionsResult> {
  const {
    filters = {},
    pageSize = 50,
    after: afterDoc,
  ...
  if (filters.isPro !== undefined) constraints.push(where('isPro', '==', filters.isPro))
...
export async function getQuestionList(type?: QuestionType, track?: Track): Promise<Pick<Question, 'id' | 'slug' | 'title' | 'category' | 'difficulty' | 'isPro' | 'order'>[]> {
  ...
  return snap.docs.map(d => {
    const data = d.data()
    return { id: d.id, slug: data.slug, title: data.title, category: data.category, difficulty: data.difficulty, isPro: data.isPro, order: data.order }
  })
}

/** Get all unique categories for a given type/track */
export async function getCategories(type?: QuestionType, track?: Track): Promise<string[]> {
...
export async function createQuestion(input: QuestionInput, adminUid: string): Promise<string> {
...
export async function updateQuestion(id: string, updates: Partial<QuestionInput>): Promise<void> {
...
export async function publishQuestion(id: string): Promise<void> {
  await updateDoc(doc(db, QUESTIONS_COL, id), { status: 'published', updatedAt: new Date().toISOString() })
}

export async function archiveQuestion(id: string): Promise<void> {
  await updateDoc(doc(db, QUESTIONS_COL, id), { status: 'archived', updatedAt: new Date().toISOString() })
}

/** Bulk update order field after drag-and-drop reordering */
export async function reorderQuestions(updates: { id: string; order: number }[]): Promise<void> {
  const batch = writeBatch(db)
  updates.forEach(({ id, order }) => {
    batch.update(doc(db, QUESTIONS_COL, id), { order, updatedAt: new Date().toISOString() })
  })
  await batch.commit()
}
...
export async function getQuestionProgress(uid: string, questionId: string): Promise<QuestionProgress | null> {
...
export async function upsertProgress(uid: string, questionId: string, update: Partial<QuestionProgress>): Promise<void> {
  const ref = doc(db, USERS_COL, uid, 'progress', questionId)
  const snap = await getDoc(ref)
  if (snap.exists()) {
    await updateDoc(ref, { ...update, lastAttemptAt: new Date().toISOString() })
  } else {
...
export async function markMasteredV2(uid: string, questionId: string, mastered: boolean): Promise<void> {
...
export async function markBookmarked(uid: string, questionId: string, bookmarked: boolean): Promise<void> {
  await upsertProgress(uid, questionId, {
    status: bookmarked ? 'bookmarked' : 'attempted',
  })
}

export async function markSolved(uid: string, questionId: string, score?: number): Promise<void> {
  await upsertProgress(uid, questionId, {
    status: 'solved',
...
export async function markRevealed(uid: string, questionId: string): Promise<void> {
...
export async function seedQuestionsFromArray(
  questions: Omit<Question, 'id' | 'viewCount' | 'solveCount' | 'createdAt' | 'updatedAt'>[],
  adminUid: string
