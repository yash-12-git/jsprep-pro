...
export const questionCard = (highlight: CardHighlight, hoverAccent = C.accent) => {
  const border = {
    idle:     C.border,
  ...
  return css`
    ...
    &:hover { border-color: ${highlight === 'idle' ? hoverAccent + '4d' : border}; }
  `
}
...
export const DIFF_STYLE: Record<string, { bg: string; color: string; border: string }> = {
