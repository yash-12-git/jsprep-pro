...
export async function generateStaticParams() {
  const cats = await getPublishedCategories().catch(() => [] as string[])
  return cats.map(cat => ({ slug: catToSlug(cat) }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const cats = await getPublishedCategories().catch(() => [] as string[])
  const cat = cats.find(c => catToSlug(c) === params.slug)
  if (!cat) return {}
  const { questions: qs } = await getQuestions({ filters: { category: cat, status: 'published' }, pageSize: 1 }).catch(() => ({ questions: [] }))
  return pageMeta({
    ...
    keywords: [`${cat.toLowerCase()} javascript interview`, `${cat.toLowerCase()} interview questions`],
  })
}

export default async function CategoryQuestionsPage({ params }: Props) {
  const allCategories = await getPublishedCategories().catch(() => [] as string[])
  const cat = allCategories.find(c => catToSlug(c) === params.slug)
  if (!cat) notFound()

  const { questions: catQuestions } = await getQuestions({
    filters: { category: cat, status: 'published', type: 'theory' },
    pageSize: 200,
  })
  const allCats = allCategories.map(c => ({ cat: c, slug: catToSlug(c) }))
  ...
  const diffBadge = (difficulty: string) => {
    if (difficulty === 'advanced' || difficulty === 'expert') return { label: 'Advanced', color: '#f7c76a' }
      ...
      <div style={{ maxWidth: '56rem', margin: '0 auto', padding: '2.5rem 1.25rem', color: '#c8c8d8' }}>
        ...
        <header style={{ marginBottom: '2.5rem' }}>
          ...
          <h1 style={{ fontSize: 'clamp(1.625rem, 4vw, 2.5rem)', fontWeight: 900, color: 'white', lineHeight: 1.2, marginBottom: '0.875rem' }}>
            ...
            <br /><span style={{ color: '#7c6af7' }}>With Answers & Code Examples</span>
          </h1>
          <p style={{ fontSize: '1rem', lineHeight: 1.75, maxWidth: '42rem', marginBottom: '1.5rem' }}>
            {catQuestions.length} carefully curated <strong style={{ color: 'white' }}>{cat} interview questions</strong> with working code examples and real interview gotchas.
          </p>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', padding: '1rem 1.25rem', background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '0.875rem', marginBottom: '2.5rem', fontSize: '0.8125rem' }}>
          ...
          <span><strong style={{ color: '#6af7c0' }}>{catQuestions.filter(q => q.difficulty === 'beginner').length}</strong> <span style={{ color: 'rgba(255,255,255,0.5)' }}>beginner</span></span>
          <span><strong style={{ color: '#7c6af7' }}>{catQuestions.filter(q => q.difficulty === 'core').length}</strong> <span style={{ color: 'rgba(255,255,255,0.5)' }}>core</span></span>
          <span><strong style={{ color: '#f7c76a' }}>{catQuestions.filter((q: Question) => q.difficulty === 'advanced' || q.difficulty === 'expert').length}</strong> <span style={{ color: 'rgba(255,255,255,0.5)' }}>advanced</span></span>
        </div>
        ...
        <main>
          {catQuestions.map((q: Question, i: number) => {
            ...
            const qSlug = q.slug
            const plainAnswer = (q.answer ?? '').replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim()
            ...
            return (
              <article key={q.id} itemScope itemType="https://schema.org/Question"
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', marginBottom: '0.875rem', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '0.6875rem', fontWeight: 800, letterSpacing: '0.06em', background: 'rgba(124,106,247,0.12)', border: '1px solid rgba(124,106,247,0.25)', color: '#7c6af7', padding: '0.125rem 0.5rem', borderRadius: '0.3125rem' }}>Q{i + 1}</span>
                  <span style={{ fontSize: '0.6875rem', fontWeight: 700, background: diff.color + '15', border: `1px solid ${diff.color}30`, color: diff.color, padding: '0.125rem 0.5rem', borderRadius: '0.3125rem' }}>{diff.label}</span>
                </div>
                ...
                <div itemProp="suggestedAnswer" itemScope itemType="https://schema.org/Answer">
                  <div itemProp="text" className="answer-body" dangerouslySetInnerHTML={{ __html: q.answer ?? '' }} />
                </div>

                {/* ── Practice → /q/[slug], never /auth ── */}
                <div style={{ marginTop: '1.25rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  <Link href={`/q/${qSlug}`} style={{ fontSize: '0.75rem', color: '#7c6af7', border: '1px solid rgba(124,106,247,0.3)', padding: '0.3125rem 0.875rem', borderRadius: '0.375rem', textDecoration: 'none', fontWeight: 700 }}>
                    Practice this question →
                  </Link>
                </div>

                {/* ── Inline AI evaluator — auth-aware ── */}
                <InlineEvaluator question={q.title} idealAnswer={plainAnswer} label="Evaluate with AI" />
              </article>
            )
          })}
        </main>
        ...
        <section style={{ marginTop: '1rem', marginBottom: '3rem' }}>
          <h2 style={{ fontSize: '1.125rem', fontWeight: 800, color: 'white', marginBottom: '1rem' }}>Other JavaScript Interview Topics</h2>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {allCats.filter(c => c.cat !== cat).map(({ cat: c, slug }) => (
              <Link key={slug} href={`/questions/${slug}`} style={{ fontSize: '0.8125rem', color: '#7c6af7', border: '1px solid rgba(124,106,247,0.25)', padding: '0.375rem 0.875rem', borderRadius: '0.5rem', textDecoration: 'none', background: 'rgba(124,106,247,0.06)' }}>
                {c} →
              </Link>
            ))}
          </div>
        ...
        <section style={{ background: 'rgba(124,106,247,0.08)', border: '1px solid rgba(124,106,247,0.2)', borderRadius: '1.25rem', padding: '2rem', textAlign: 'center', marginBottom: '2rem' }}>
          <h2 style={{ fontSize: '1.375rem', fontWeight: 900, color: 'white', marginBottom: '0.625rem' }}>Ready to practice {cat}?</h2>
