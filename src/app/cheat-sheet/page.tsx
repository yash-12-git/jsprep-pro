...
export default function CheatSheetPage() {
  ...
  const masteredQs = questions.filter(q =>
    includeAll ? true : progress.masteredIds.includes(q.id)
  )
    ...
    <>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-muted text-sm">Auto-generates a printable 1-page PDF of your mastered concepts</p>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-6 mb-6">
              ...
              <div>
                ...
                <p className="text-muted text-sm">
                  {includeAll
                    ...
                    : `${progress.masteredIds.length} mastered questions`}
                </p>
              </div>
            ...
            {progress.masteredIds.length === 0 && !includeAll && (
              <div className="bg-accent2/10 border border-accent2/20 rounded-xl p-4 mb-4 text-sm text-accent2">
                ⚠️ You haven't mastered any questions yet. Switch to "All Questions" or go mark some questions as mastered first.
              </div>
            )}
            ...
            <p className="text-muted text-xs text-center mt-3">
              In the print dialog, select "Save as PDF" to download. Works in Chrome, Safari, and Firefox.
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-muted text-xs">({masteredQs.length} questions, {Object.keys(byCategory).length} categories)</span>
            </div>
            <div className="bg-white rounded-xl p-5 text-black max-h-96 overflow-y-auto text-xs">
              <div className="text-center mb-4 border-b border-gray-200 pb-3">
                <h1 className="text-lg font-black text-gray-900">JavaScript Interview Cheat Sheet</h1>
                <p className="text-gray-500 text-xs">{user.displayName} · Generated {new Date().toLocaleDateString()} · JSPrep Pro</p>
              </div>
              {Object.entries(byCategory).map(([cat, qs]) => (
                <div key={cat} className="mb-4">
                  <h2 className="font-black text-purple-700 text-sm border-b border-purple-200 pb-1 mb-2">{cat}</h2>
                  {qs.map(q => (
                    <div key={q.id} className="mb-3">
                      <p className="font-bold text-gray-800 mb-0.5">Q: {q.q}</p>
                      {q.hint && <p className="text-gray-500 italic text-[10px] mb-0.5">💡 {q.hint}</p>}
                    </div>
                  ))}
                </div>
              ))}
      ...
      <div ref={printRef} className="hidden print:block">
        ...
        <div className="print-header">
          ...
          <p>{user.displayName} · {user.email} · Generated {new Date().toLocaleDateString('en-IN')} · JSPrep Pro</p>
        </div>
          ...
          <div key={cat}>
            ...
            {qs.map(q => (
              <div key={q.id} className="q-item">
                <p className="q-text">Q: {q.q}</p>
                {q.hint && <p className="q-hint">💡 Hint: {q.hint}</p>}
                <div className="q-tags" style={{marginBottom: '4px'}}>
                  {q.tags.map(t => <span key={t} className="tag">{t.toUpperCase()}</span>)}
                </div>
                <div
                  className="q-answer"
                  dangerouslySetInnerHTML={{
                    __html: q.answer
                      ...
                      .replace(/<div class="tip">/g, '<div style="background:#f0fff4;border-left:2px solid #6af7a0;padding:4px 6px;border-radius:0 4px 4px 0;margin-top:4px;font-size:9px;color:#333">')
                  }}
                />
              </div>
            ))}
