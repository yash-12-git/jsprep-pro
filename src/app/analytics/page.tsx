...
export default function AnalyticsPage() {
  ...
  const { questions: theoryQs } = useQuestions({ type: 'theory',  track: 'javascript', enabled: !!user })
  const { questions: outputQs } = useQuestions({ type: 'output',  track: 'javascript', enabled: !!user })
  const { questions: debugQs  } = useQuestions({ type: 'debug',   track: 'javascript', enabled: !!user })

  const { progressMap, masteredIds, solvedIds } = useUserProgress({ uid: user?.uid ?? null })
  ...
  if (!progress.isPro) return (
    <><Navbar /><PaywallBanner reason="Analytics is a Pro feature. Upgrade to see your full progress breakdown!" /></>
  )

  // Compute counts from Firestore progress map
  const masteredCount     = masteredIds.length
  const solvedOutputCount = outputQs.filter(q => solvedIds.includes(q.id)).length
  const solvedDebugCount  = debugQs.filter(q => solvedIds.includes(q.id)).length
  ...
  const masterPct         = theoryQs.length > 0 ? Math.round((masteredCount / theoryQs.length) * 100) : 0
  const outputPct         = outputQs.length  > 0 ? Math.round((solvedOutputCount / outputQs.length) * 100) : 0
  const debugPct          = debugQs.length   > 0 ? Math.round((solvedDebugCount / debugQs.length) * 100) : 0
  ...
  const statCards = [
    ...
    { label: 'Theory',      value: `${masteredCount}/${theoryQs.length}`, icon: Brain, color: C.accent,  bg: `${C.accent}1a`  },
    { label: 'Output Quiz', value: `${solvedOutputCount}/${outputQs.length}`, icon: Code2, color: C.accent2, bg: `${C.accent2}1a` },
    { label: 'Debug Lab',   value: `${solvedDebugCount}/${debugQs.length}`,   icon: Bug,   color: C.danger,  bg: `${C.danger}1a`  },
  ]
      ...
      <div css={Shared.pageWrapper}>
        ...
        <div css={S.section}>
          ...
          <div css={S.categoryRow}>
            {[
              { label:'📖 Theory Questions', pct:masterPct, solved:masteredCount,     total:theoryQs.length, color:C.accent  },
              { label:'💻 Output Quiz',       pct:outputPct, solved:solvedOutputCount, total:outputQs.length, color:C.accent2 },
              { label:'🐛 Debug Lab',         pct:debugPct,  solved:solvedDebugCount,  total:debugQs.length,  color:C.danger  },
        ...
        {[
          { title:'📖 Theory — Category Breakdown',     sub:"Which JS concepts you've mastered", stats:catStats },
          { title:'💻 Output Quiz — Category Breakdown', sub:'Where you predict code output correctly', stats:outputCatStats },
          { title:'🐛 Debug Lab — Category Breakdown',   sub:'Which bug types you can spot and fix', stats:debugCatStats },
        ].map(({ title, sub, stats }) => stats.length > 0 && (
              ...
              {quizHistory.map((q, i) => {
                ...
                return (
                  <div key={i} css={S.quizHistoryRow}>
                    ...
                    <div css={S.barTrack}><div css={S.barFill(p, barColor(p))} /></div>
