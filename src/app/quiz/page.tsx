...
export default function QuizPage() {
  ...
  if (authLoading || qLoading || !user || !progress)
    return (
  ...
  function startQuiz() {
    const selected = shuffle(questions).slice(
      0,
  ...
  function handleAnswer(correct: boolean) {
    ...
    if (current + 1 >= quizQuestions.length) {
      saveQuizScore(
        user.uid,
        newScores.filter(Boolean).length,
    ...
    <>
      <div css={Shared.pageWrapper}>
        {phase === "setup" && (
          <div css={{ textAlign: "center" }}>
            ...
            <div css={[Shared.card, { padding: "2rem", marginBottom: "2rem" }]}>
              ...
              >
                {[5, 10, 15, Math.min(20, questions.length)]
        ...
        {phase === "quiz" && q && (
          <div>
            <div css={S.header}>
              <span css={S.questionCounter}>
                {current + 1} / {quizQuestions.length}
              </span>
              <div css={S.timerBox}>
                <span css={S.timerText(timeLeft <= 10)}>
            ...
            {!revealed ? (
              <button
                css={[Shared.ghostBtn, { width: "100%", padding: "1rem" }]}
        ...
        {phase === "result" && (
          <div css={S.resultsCard}>
            ...
            <p css={{ color: C.muted, marginBottom: "2rem" }}>
              Here's how you did
            </p>
            ...
            <div
              css={{ display: "flex", gap: "1rem", justifyContent: "center" }}
            >
              <button
                css={Shared.primaryBtn(C.accent)}
