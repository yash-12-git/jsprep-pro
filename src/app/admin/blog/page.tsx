...
import { PlusCircle, Edit2, Trash2, Eye, EyeOff, Search, RefreshCw, ExternalLink } from 'lucide-react'
import { getBlogPosts, deleteBlogPost, publishBlogPost, archiveBlogPost } from '@/lib/blogPosts'
...
const S = {
  ...
  dot: (color: string) => css`width:8px;height:8px;border-radius:50%;background:${color};display:inline-block;margin-right:0.375rem;`,
  ...
  linkWarn: css`font-size:0.6875rem;color:${C.accent2};background:${C.accent2}0d;border:1px solid ${C.accent2}25;border-radius:0.25rem;padding:0.1rem 0.5rem;margin-left:0.375rem;`,
}
...
export default function AdminBlogPage() {
  ...
  const unlinked = posts.filter(p => !p.topicSlug && p.status === 'published').length
    ...
    <div>
      <div css={S.topRow}>
        <div>
          ...
          {unlinked > 0 && (
            <p style={{ fontSize: '0.75rem', color: C.accent2, marginTop: '0.25rem' }}>
              ⚠ {unlinked} published post{unlinked !== 1 ? 's' : ''} not linked to a topic — edit them to set topicSlug.
            </p>
          )}
        </div>
      ...
      <div css={S.searchBar}>
        ...
        <input css={S.searchInput} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search posts…" />
      </div>

      <div css={S.meta}>{loading ? 'Loading…' : `${filtered.length} of ${posts.length} posts`}</div>

      {!loading && filtered.length === 0 ? (
        <div css={S.empty}>
          {posts.length === 0
            ? <>No posts yet. <Link href="/admin/migrate" style={{ color: C.accent }}>Run migration</Link> to import from static files.</>
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={p.id} css={S.tr}>
                <td css={S.td}>
                  <div style={{ fontWeight: 600, color: 'white', marginBottom: '0.1875rem' }}>{p.title}</div>
                  <div style={{ fontSize: '0.6875rem', color: C.muted }}>{p.readTime} · {p.publishedAt}</div>
                </td>
                <td css={S.td}>
                  <span css={S.badge(p.accentColor ?? C.accent)}>{p.category}</span>
                </td>
                <td css={S.td}>
                  {p.topicSlug
                    ? <span css={S.badge(C.accent3)}>{p.topicSlug.replace('javascript-', '').replace('-interview-questions', '')}</span>
                    : <span css={S.badge(C.muted)}>not linked</span>
                  }
                  {p.relatedTopicSlugs?.length > 0 && (
                    <span style={{ fontSize: '0.6875rem', color: C.muted, marginLeft: '0.375rem' }}>
                      +{p.relatedTopicSlugs.length}
                    </span>
                  )}
                </td>
                <td css={S.td}>
                  <span css={S.badge(p.status === 'published' ? C.accent3 : p.status === 'draft' ? C.muted : C.danger)}>
                    {p.status}
                  </span>
                </td>
                <td css={S.td}>
                  <div css={S.actions}>
                    <Link href={`/blog/${p.slug}`} target="_blank" css={S.iconBtn(C.accent)} title="View live">
                      <ExternalLink size={13} />
                    </Link>
                    <Link href={`/admin/blog/${p.id}`} css={S.iconBtn(C.accent2)} title="Edit">
                      <Edit2 size={13} />
                    </Link>
                    <button css={S.iconBtn(p.status === 'published' ? C.muted : C.accent3)}
                      onClick={() => handleToggleStatus(p)}
                      title={p.status === 'published' ? 'Unpublish' : 'Publish'}>
                      {p.status === 'published' ? <EyeOff size={13} /> : <Eye size={13} />}
                    </button>
                    <button css={S.iconBtn(C.danger)} onClick={() => handleDelete(p.id, p.title)} title="Delete">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </td>
