...
const S = {
  ...
  heading: css`font-size: 1.5rem; font-weight: 900; margin-bottom: 0.375rem; color: ${C.text};`,
  ...
  dataTitle: css`font-size: 0.9375rem; font-weight: 800; color: ${C.text}; margin-bottom: 0.25rem;`,
  ...
  embedBtn: (loading: boolean) => css`
    ...
    border: 1px solid ${C.accent3}44; border-radius: ${RADIUS.xl};
    color: ${C.accent3}; font-weight: 700; font-size: 0.875rem;
    cursor: ${loading ? 'not-allowed' : 'pointer'};
    transition: all 0.15s;
    &:hover { background: ${loading ? '' : C.accent3 + '28'}; }
  `,
  ...
  logLine: (type: 'info' | 'success' | 'error' | 'warn') => {
    const colors = { info: C.muted, success: C.green, error: C.red, warn: C.amber }
...
export default function SeedPage() {
  ...
  async function seedBatch(
    ...
    for (let i = 0; i < questions.length; i += BATCH_SIZE) {
      ...
      await batch.commit()
      addLog('success', `  ${label}: committed ${Math.min(i + BATCH_SIZE, questions.length)}/${questions.length}`)
    }
  ...
  function buildEmbedText(q: QuestionInput): string {
    ...
    const parts = [q.title, q.category ?? '', q.explanation ?? '', q.keyInsight ?? '']
    if (q.type === 'output') parts.push(q.code ?? '', q.expectedOutput ?? '')
    if (q.type === 'debug')  parts.push(q.brokenCode ?? '', q.bugDescription ?? '')
    return parts.filter(Boolean).join(' ').slice(0, 4000)
  }

  async function generateEmbeddings(questions: QuestionInput[]) {
    const toEmbed = questions.filter(q => q.type === 'output' || q.type === 'debug')
    addLog('info', `Generating embeddings for ${toEmbed.length} output/debug questions...`)
  ...
  async function handleReEmbed() {
    ...
    try {
      ...
      const all = snap.docs.map(d => ({ ...d.data(), slug: d.data().slug })) as QuestionInput[]
      const toEmbed = all.filter((q: any) => (q.type === 'output' || q.type === 'debug') && !q.embedding?.length)
      ...
      for (const q of toEmbed) {
        try {
          ...
          if (!snap2.empty) {
            ...
            addLog('success', `  [${done}/${toEmbed.length}] ${(q as any).title}`)
          }
          await new Promise(r => setTimeout(r, 150))
        } catch (e: any) {
      ...
      addLog('success', `Re-embed complete — ${done}/${toEmbed.length} updated`)
    } catch (e: any) {
      addLog('error', `Error: ${e.message}`)
    } finally {
      setEmbedding(false)
  ...
  async function handleClearAll() {
    ...
    try {
      ...
      for (let i = 0; i < snap.docs.length; i += BATCH_SIZE) {
        ...
        await batch.commit()
        addLog('info', `  Deleted ${Math.min(i + BATCH_SIZE, snap.docs.length)}/${snap.docs.length}`)
      }
  ...
  return (
    <div css={S.page}>
      ...
      <p css={S.sub}>
        Import static question files into Firestore. Run once after setup.
        Use <strong>Clear All</strong> before re-seeding to avoid duplicates.
        <br />
        Embeddings are generated automatically after seeding — required for the dedup pipeline in the AI generator.
      </p>
      ...
      {/* Action buttons */}
      <div css={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1rem' }}>
        <button css={S.seedBtn(seeding)} onClick={handleSeed} disabled={isBusy}>
          {seeding
            ? <><Loader2 size={16} css={{ animation: 'spin 1s linear infinite' }} /> {embedding ? 'Generating embeddings…' : 'Seeding…'}</>
            : <><Database size={16} /> Seed {totalQs} Questions + Embeddings</>
          }
        </button>

        <button css={S.embedBtn(embedding)} onClick={handleReEmbed} disabled={isBusy}>
          {embedding
            ? <><Loader2 size={14} css={{ animation: 'spin 1s linear infinite' }} /> Embedding…</>
      ...
      <ol style={{ paddingLeft: '1.25rem', fontSize: '0.875rem', lineHeight: 2.1, color: C.text }}>
        ...
        <li>
          ...
          <pre style={{ background: 'var(--color-code-bg, #f7f7f5)', border: `1px solid ${C.border}`, borderRadius: '0.5rem', padding: '0.75rem', marginTop: '0.5rem', fontSize: '0.75rem', color: C.text, overflowX: 'auto' }}>
...
Index 4:  status ASC + category ASC + order ASC`}
        ...
        <li>Open your <a href="/dashboard" style={{ color: C.accent }}>dashboard</a> — questions now load from Firestore.</li>
        <li>
          If embedding generation failed for some questions, click <strong>Re-Embed Missing</strong> to retry.
          The AI generator's dedup feature requires embeddings on all output + debug questions.
        </li>
      </ol>
