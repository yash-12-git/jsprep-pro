...
import {
  ...
  BookOpen, Code2, Bug, Trash2, RefreshCw
} from 'lucide-react'
...
// ─── Tag → difficulty map ─────────────────────────────────────────────────────
const tagToDifficulty = (tags: string[]): 'beginner' | 'core' | 'advanced' | 'expert' => {
...
function buildTheoryQuestions(): QuestionInput[] {
  return theoryRaw.map((q, i) => ({
    slug: q.q.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 60),
...
function buildOutputQuestions(): QuestionInput[] {
  return outputQuestions.map((q, i) => ({
    slug: `output-${q.id}-${q.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 40)}`,
...
function buildDebugQuestions(): QuestionInput[] {
  return debugQuestions.map((q, i) => ({
    slug: `debug-${q.id}-${q.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 40)}`,
    type:        'debug' as const,
    track:       'javascript' as const,
    title:       q.title,
...
const S = {
  ...
  sub: css`color: ${C.muted}; font-size: 0.875rem; margin-bottom: 2rem; line-height: 1.7;`,
  ...
  dataTitle: css`font-size: 0.9375rem; font-weight: 800; color: white; margin-bottom: 0.25rem;`,
  dataCount: (color: string) => css`font-size: 1.5rem; font-weight: 900; color: ${color};`,
  dataMeta: css`font-size: 0.6875rem; color: ${C.muted}; margin-top: 0.25rem;`,

  doneBadge: (color: string) => css`
    display: inline-flex; align-items: center; gap: 0.25rem;
    font-size: 0.625rem; font-weight: 800; text-transform: uppercase;
    background: ${color}14; border: 1px solid ${color}33; color: ${color};
    padding: 0.125rem 0.5rem; border-radius: 0.25rem; margin-top: 0.5rem;
  `,
  ...
  dangerBtn: css`
    ...
    color: ${C.danger}; font-size: 0.875rem; font-weight: 700;
    cursor: pointer;
  ...
  logLine: (type: 'info' | 'success' | 'error' | 'warn') => {
    const colors = { info: C.muted, success: C.accent3, error: C.danger, warn: C.accent2 }
    return css`color: ${colors[type]};`
  },
  ...
  sectionTitle: css`
    ...
    color: ${C.muted}; margin-bottom: 0.875rem;
  `,
}

// ─── Log entry type ───────────────────────────────────────────────────────────

interface LogEntry { type: 'info' | 'success' | 'error' | 'warn'; text: string }
...
export default function SeedPage() {
  ...
  async function seedBatch(
    ...
    for (let i = 0; i < questions.length; i += BATCH_SIZE) {
      ...
      await batch.commit()
      addLog('success', `  ✓ ${label}: committed batch (${Math.min(i + BATCH_SIZE, questions.length)}/${questions.length})`)
    }
  ...
  async function handleSeed() {
    ...
    try {
      ...
      addLog('info', '─────────────────────────────────')
      addLog('success', `🎉 Seed complete! ${totalQs} questions are now in Firestore.`)
      addLog('warn', 'Go to /admin/questions to verify. Then update Firestore indexes if queries fail.')
    } catch (e: any) {
  ...
  async function handleClearAll() {
    if (!user) return
    if (!window.confirm('Delete ALL questions from Firestore? This cannot be undone.')) return
    setClearing(true)
    setLog([])
    ...
    try {
      ...
      for (let i = 0; i < snap.docs.length; i += BATCH_SIZE) {
        ...
        await batch.commit()
        addLog('info', `  Deleted batch ${Math.min(i + BATCH_SIZE, snap.docs.length)}/${snap.docs.length}`)
      }
  ...
  return (
    <div css={S.page}>
      ...
      <p css={S.sub}>
        Migrate your existing static <code style={{ background: C.surface, padding: '0.1rem 0.4rem', borderRadius: '0.25rem', fontSize: '0.8125rem', color: C.accent3 }}>.ts</code> question files
        into Firestore in one click. Run this once. After seeding, manage questions via the admin panel — no more code changes needed to add/edit questions.
      </p>
      ...
      <div css={S.grid}>
        {[
          { label: 'Theory Questions', count: theoryQs.length,  icon: BookOpen, color: C.accent,  desc: 'Core JS, Functions, Async, Objects…', key: 'Theory' },
          { label: 'Output Questions', count: outputQs.length,  icon: Code2,   color: C.accent2, desc: 'Event loop, closures, type coercion…', key: 'Output' },
          { label: 'Debug Questions',  count: debugQs.length,   icon: Bug,     color: C.danger,  desc: 'Async bugs, closure traps, fix the code…', key: 'Debug' },
      ...
      <div css={S.warningBox}>
        ⚠️ <strong>Run this only once.</strong> If you run it again it will create duplicate questions.
        Use "Clear All" first if you need to re-seed.
      </div>

      {/* Seed button */}
      <div css={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
        <button css={S.seedBtn(seeding)} onClick={handleSeed} disabled={seeding || clearing}>
          {seeding
            ? <><Loader2 size={16} css={{ animation: 'spin 1s linear infinite' }} /> Importing…</>
            : <><Database size={16} /> Import {totalQs} Questions to Firestore</>
          }
        </button>

        <button css={S.dangerBtn} onClick={handleClearAll} disabled={seeding || clearing}>
          {clearing
            ? <><Loader2 size={14} css={{ animation: 'spin 1s linear infinite' }} /> Clearing…</>
      ...
      <ol style={{ paddingLeft: '1.25rem', fontSize: '0.875rem', lineHeight: 2.1, color: C.text }}>
        <li>Go to <a href="/admin/questions" style={{ color: C.accent }}>Admin → All Questions</a> to verify all rows appear.</li>
        <li>
          In Firebase Console → Firestore → <strong>Indexes</strong>, create these composite indexes:
          <pre style={{ background: '#0a0a10', border: `1px solid ${C.border}`, borderRadius: '0.5rem', padding: '0.75rem', marginTop: '0.5rem', fontSize: '0.75rem', color: '#c8c8d8', overflowX: 'auto' }}>
...
Index 4:  status ASC + category ASC + order ASC`}
        ...
        <li>Open your <a href="/dashboard" style={{ color: C.accent }}>dashboard</a> — it should now load questions from Firestore.</li>
        <li>Your old <code style={{ color: C.accent3, fontSize: '0.8125rem' }}>src/data/questions.ts</code> is still imported here but no longer used by the app. You can keep it as a backup.</li>
      </ol>
