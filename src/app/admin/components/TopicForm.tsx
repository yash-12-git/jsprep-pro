...
import type { Topic, TopicInput, TopicDifficulty, TopicStatus } from '@/types/topic'
...
const S = {
  ...
  arrayField: css`
    ...
    border-radius: ${RADIUS.md}; padding: 0.625rem;
  ...
  arrayInput: css`
    ...
    font-size: 0.8125rem; color: white; padding: 0.25rem 0;
    resize: none; line-height: 1.5;
    &::placeholder { color: rgba(255,255,255,0.2); }
  `,

  removeBtn: css`
    flex-shrink: 0; padding: 0.1875rem; margin-top: 0.25rem;
  ...
  addItemBtn: css`
    ...
    padding: 0.3125rem 0.625rem; background: none;
    border: 1px dashed rgba(255,255,255,0.1); border-radius: ${RADIUS.sm};
    color: ${C.muted}; font-size: 0.75rem; font-weight: 600; cursor: pointer;
    transition: all 0.15s;
    &:hover { border-color: ${C.accent}55; color: ${C.accent}; }
  `,
...
const EMPTY: TopicInput = {
  slug: '', title: '', category: '', keyword: '', description: '',
...
export default function TopicForm({ mode, initial = {}, onSubmit, onDelete }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} css={S.wrap}>
      {success && (
        <div css={S.success}><CheckCircle size={16} /> {mode === 'create' ? 'Topic created!' : 'Changes saved!'}</div>
      )}
      ...
      <div css={S.row}>
        <div>
          <label css={S.label}>Keyword <span css={S.required}>*</span><span css={S.labelNote}>e.g. closure, hoisting</span></label>
          <input css={S.input} value={form.keyword} onChange={e => set('keyword', e.target.value)} placeholder="closure" />
        </div>
        <div>
          <label css={S.label}>Category <span css={S.required}>*</span><span css={S.labelNote}>matches Question.category</span></label>
          <input css={S.input} value={form.category} onChange={e => set('category', e.target.value)} placeholder="Functions" />
        </div>
      </div>

      <div css={S.field}>
        <label css={S.label}>Title <span css={S.required}>*</span></label>
        <input css={S.input} value={form.title} onChange={e => set('title', e.target.value)} placeholder="JavaScript Closure Interview Questions" />
      </div>

      <div css={S.row}>
        <div>
          <label css={S.label}>Slug (URL)</label>
          <input css={S.input} value={form.slug} onChange={e => set('slug', e.target.value)} placeholder="auto-generated from keyword" />
        </div>
        <div>
          <label css={S.label}>Question Count<span css={S.labelNote}>display only</span></label>
          <input css={S.input} value={form.questionCount} onChange={e => set('questionCount', e.target.value)} placeholder="8–12" />
        </div>
      </div>

      <div css={S.field}>
        <label css={S.label}>Description <span css={S.required}>*</span></label>
        <textarea css={S.textarea} rows={2} value={form.description} onChange={e => set('description', e.target.value)} placeholder="One-line description shown under the H1 + used in meta." />
      </div>

      <div css={S.row3}>
        <div>
          <label css={S.label}>Difficulty</label>
          <select css={S.select} value={form.difficulty} onChange={e => set('difficulty', e.target.value as TopicDifficulty)}>
        ...
        <div>
          ...
          <select css={S.select} value={form.status} onChange={e => set('status', e.target.value as TopicStatus)}>
        ...
        <div>
          ...
          <input css={S.input} type="number" min={0} value={form.order ?? 0} onChange={e => set('order', Number(e.target.value))} />
        </div>
      </div>
      ...
      <div css={S.footer}>
        ...
        <button type="submit" css={S.submitBtn(saving)} disabled={saving}>
          {saving
            ...
            : <><Save size={16} /> {mode === 'create' ? 'Create Topic' : 'Save Changes'}</>}
        </button>
      </div>
