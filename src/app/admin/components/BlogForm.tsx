...
const S = {
  ...
  label: css`
    ...
    color: ${C.muted}; margin-bottom: 0.375rem;
  `,

  labelNote: css`
    font-weight: 500; text-transform: none; letter-spacing: 0;
    color: ${C.muted}; margin-left: 0.5rem;
  `,
  ...
  input: css`
    ...
    border-radius: ${RADIUS.md}; padding: 0.625rem 0.875rem;
  ...
  select: css`
    width: 100%; background: ${C.surface}; border: 1px solid ${C.border};
    border-radius: ${RADIUS.md}; padding: 0.625rem 0.875rem;
    font-size: 0.875rem; color: white; outline: none; cursor: pointer;
    appearance: none;
  ...
  textarea: css`
    width: 100%; background: ${C.surface}; border: 1px solid ${C.border};
    border-radius: ${RADIUS.md}; padding: 0.625rem 0.875rem;
    font-size: 0.875rem; color: white; outline: none; resize: vertical;
    &:focus { border-color: ${C.accent}55; }
    &::placeholder { color: rgba(255,255,255,0.2); }
  `,

  divider: css`border: none; border-top: 1px solid ${C.border}; margin: 1.5rem 0;`,

  sectionLabel: css`
    font-size: 0.6875rem; font-weight: 900; text-transform: uppercase;
    letter-spacing: 0.1em; color: ${C.muted}; margin-bottom: 1rem;
  `,
  ...
  topicChipText: css`font-size: 0.8125rem; color: white;`,
  topicChipMeta: css`font-size: 0.6875rem; color: ${C.muted};`,

  // Tags input for question categories
  tagsInput: css`
    display: flex; flex-wrap: wrap; gap: 0.375rem;
    background: ${C.surface}; border: 1px solid ${C.border};
    border-radius: ${RADIUS.md}; padding: 0.5rem 0.75rem;
  ...
  tag: css`
    ...
    color: ${C.accent}; font-size: 0.6875rem; font-weight: 700;
    padding: 0.125rem 0.5rem; border-radius: 0.25rem; cursor: pointer;
    &:hover { background: ${C.danger}18; border-color: ${C.danger}33; color: ${C.danger}; }
  `,

  tagInput: css`
    flex: 1; min-width: 8rem; background: transparent;
    border: none; outline: none; font-size: 0.875rem; color: white;
    &::placeholder { color: rgba(255,255,255,0.2); }
  `,

  toggle: (active: boolean, color: string) => css`
    display: inline-flex; align-items: center; gap: 0.375rem;
    padding: 0.375rem 0.875rem; border-radius: ${RADIUS.md};
  ...
  footer: css`
    ...
    gap: 1rem; padding-top: 1.5rem; border-top: 1px solid ${C.border}; margin-top: 1.5rem;
  `,
  ...
  deleteBtn: css`
    ...
    border: 1px solid ${C.danger}44; border-radius: ${RADIUS.xl};
    color: ${C.danger}; font-size: 0.8125rem; font-weight: 700;
  ...
  success: css`
    ...
    background: ${C.accent3}12; border: 1px solid ${C.accent3}33;
    border-radius: ${RADIUS.xl}; color: ${C.accent3};
    font-size: 0.875rem; font-weight: 700; margin-bottom: 1.5rem;
  `,

  error: css`
    padding: 0.875rem 1.25rem;
    background: ${C.danger}12; border: 1px solid ${C.danger}33;
    border-radius: ${RADIUS.xl}; color: ${C.danger};
...
const EMPTY: BlogPostInput = {
  slug: '', title: '', excerpt: '',
  category: 'Deep Dive', accentColor: '#7c6af7',
  readTime: '5 min read',
...
const ACCENT_PRESETS = [
  '#7c6af7', '#6af7c0', '#f7c76a', '#f76a6a', '#60a5fa', '#a78bfa',
]

const CATEGORIES = [
  'Deep Dive', 'Interview Prep', 'Core Concepts',
  'Modern JS', 'Best Practices', 'Practice',
]

// ─── Component ────────────────────────────────────────────────────────────────

export default function BlogForm({ mode, initial = {}, onSubmit, onDelete }: Props) {
  ...
  useEffect(() => {
    getPublishedTopics().then(setTopics).catch(() => {})
  }, [])
  ...
  function removeCat(c: string) {
    set('questionCategories', form.questionCategories.filter(x => x !== c))
  }
  ...
  function removeKeyword(k: string) {
    set('keywords', form.keywords.filter(x => x !== k))
  }
  ...
  return (
    <form onSubmit={handleSubmit} css={S.wrap}>
      {success && (
        <div css={S.success}><CheckCircle size={16} /> {mode === 'create' ? 'Post created!' : 'Changes saved!'}</div>
      )}
      ...
      <div css={S.field}>
        ...
        <input css={S.input} value={form.title} onChange={e => set('title', e.target.value)}
      ...
      <div css={S.row}>
        <div>
          ...
          <input css={S.input} value={form.slug} onChange={e => set('slug', e.target.value)}
      ...
      <div css={S.field}>
        <label css={S.label}>Excerpt <span css={S.required}>*</span></label>
        <textarea css={S.textarea} rows={2} value={form.excerpt}
      ...
      <div css={S.row3}>
        <div>
          ...
          <input css={S.input} value={form.readTime} onChange={e => set('readTime', e.target.value)} placeholder="8 min read" />
        </div>
        <div>
          <label css={S.label}>Published</label>
          <input css={S.input} type="date" value={form.publishedAt} onChange={e => set('publishedAt', e.target.value)} />
        </div>
        <div>
          <label css={S.label}>Last Modified</label>
          <input css={S.input} type="date" value={form.modifiedAt} onChange={e => set('modifiedAt', e.target.value)} />
        </div>
      </div>
          ...
          {ACCENT_PRESETS.map(c => (
            <button key={c} type="button"
              ...
              style={{
                ...
                background: c, border: form.accentColor === c ? `3px solid white` : `2px solid transparent`,
                cursor: 'pointer', transition: 'border 0.1s',
              }}
            />
          ))}
          <input css={[S.input, { width: '7rem' }]} value={form.accentColor}
            onChange={e => set('accentColor', e.target.value)} placeholder="#7c6af7" />
        </div>
      </div>
      ...
      <div css={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <select css={[S.select, { width: 'auto', padding: '0.375rem 2rem 0.375rem 0.75rem' }]}
          value={form.status} onChange={e => set('status', e.target.value as BlogStatus)}>
      ...
      <div css={S.sectionLabel}>Topic Linking — connects this post to topic pages</div>

      {/* Primary topic */}
      <div css={S.field}>
        <label css={S.label}>
          Primary Topic<span css={S.labelNote}>— shown as "Read More" on this topic's page</span>
        </label>
      ...
      <div css={S.field}>
        <label css={S.label}>
          Related Topics<span css={S.labelNote}>— shown in "Practice These Topics" sidebar</span>
        </label>
        {topics.length === 0 ? (
          <p style={{ fontSize: '0.8125rem', color: C.muted }}>
            No topics found. <a href="/admin/migrate" style={{ color: C.accent }}>Run migration</a> first.
          </p>
        ) : (
          <div css={S.topicsGrid}>
            {topics.map(t => {
              ...
              return (
                <button key={t.slug} type="button" css={S.topicChip(selected)}
                  onClick={() => toggleRelatedTopic(t.slug)}>
                  <div>
                    <div css={S.topicChipText}>{t.keyword}</div>
                    <div css={S.topicChipMeta}>{t.category}</div>
                  </div>
                  {selected && <X size={12} style={{ marginLeft: 'auto', color: C.accent, flexShrink: 0 }} />}
                </button>
              )
            })}
          </div>
      ...
      <div css={S.field}>
        <label css={S.label}>
          Question Categories<span css={S.labelNote}>— pull related questions into sidebar</span>
        </label>
        <div css={S.tagsInput}>
          {form.questionCategories.map(c => (
            <span key={c} css={S.tag} onClick={() => removeCat(c)}>{c} ×</span>
          ))}
          <input css={S.tagInput} value={catInput}
      ...
      <div css={S.field}>
        ...
        <div css={S.tagsInput}>
          {form.keywords.map(k => (
            <span key={k} css={S.tag} onClick={() => removeKeyword(k)}>{k} ×</span>
          ))}
          <input css={S.tagInput} value={kwInput}
            onChange={e => setKwInput(e.target.value)}
            onKeyDown={e => {
              if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addKeyword(kwInput) }
              if (e.key === 'Backspace' && !kwInput && form.keywords.length) removeKeyword(form.keywords[form.keywords.length - 1])
            }}
      ...
      <div css={S.footer}>
        <div css={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          {showDeleteConfirm && (
            <button type="button" onClick={() => setShowDeleteConfirm(false)}
              style={{ fontSize: '0.75rem', color: C.muted, background: 'none', border: 'none', cursor: 'pointer' }}>
              Cancel
            </button>
          )}
        </div>
        ...
        <button type="submit" css={S.submitBtn(saving)} disabled={saving}>
          {saving
            ...
            : <><Save size={16} /> {mode === 'create' ? 'Create Post' : 'Save Changes'}</>}
        </button>
      </div>
