    ...
    <form onSubmit={handleSubmit} css={S.wrap}>
      ...
      {isPolyfill && (
        <>
          <div css={S.field}>
            <label css={S.label}>
              Method Name <span css={S.required}>*</span>
            </label>
            <input
              css={S.input}
              value={(form as any).methodName ?? ""}
              onChange={(e) => set("methodName" as any, e.target.value)}
              placeholder="Array.prototype.myReduce"
            />
            <span css={S.labelHint}>
              The full method path shown to the user (e.g.
              Array.prototype.myMap, Promise.myAll, debounce)
            </span>
          </div>
          <div css={S.field}>
            <label css={S.label}>
              Starter Code <span css={S.required}>*</span>
            </label>
            <textarea
              css={S.textarea}
              rows={8}
              value={(form as any).starterCode ?? ""}
              onChange={(e) => set("starterCode" as any, e.target.value)}
              placeholder={
                "Array.prototype.myMap = function(callback) { // TODO: implement };"
              }
            />
            <span css={S.labelHint}>
              What the user sees in Monaco — function signature + TODO comment
            </span>
          </div>
          <div css={S.field}>
            <label css={S.label}>
              Test Code <span css={S.required}>*</span>
            </label>
            <textarea
              css={S.textarea}
              rows={10}
              value={(form as any).testCode ?? ""}
              onChange={(e) => set("testCode" as any, e.target.value)}
              placeholder={
                "test('doubles values', () => { expect([1,2,3].myMap(x => x*2)).toEqual([2,4,6]); });"
              }
            />
            <span css={S.labelHint}>
              Uses test() / expect() framework. Injected after user code. Each
              test() call = 1 test.
            </span>
          </div>
          <div css={S.field}>
            <label css={S.label}>
              Test Count <span css={S.required}>*</span>
            </label>
            <input
              css={S.input}
              type="number"
              value={(form as any).testCount ?? 0}
              onChange={(e) => set("testCount" as any, Number(e.target.value))}
              placeholder="5"
            />
            <span css={S.labelHint}>
              Must match the number of test() calls in Test Code above
            </span>
          </div>
          <div css={S.field}>
            <label css={S.label}>
              Solution Code <span css={S.required}>*</span>
            </label>
            <textarea
              css={S.textarea}
              rows={10}
              value={(form as any).solutionCode ?? ""}
              onChange={(e) => set("solutionCode" as any, e.target.value)}
          ...
          <div css={S.field}>
            ...
            <textarea
              css={S.textarea}
              rows={4}
              value={((form as any).hints ?? []).join("")}
              onChange={(e) =>
                set("hints" as any, e.target.value.split("").filter(Boolean))
              }
              placeholder={
                "Declare let timer = null in the closure On each call, clearTimeout(timer) then set a new timer"
              }
            />
            <span css={S.labelHint}>
              Progressive hints — shown one at a time when user clicks "Get a
              hint"
            </span>
          </div>
        </>
