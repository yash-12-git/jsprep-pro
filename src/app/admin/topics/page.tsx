...
export default function AdminTopicsPage() {
  ...
  async function handleToggleStatus(topic: Topic) {
    if (topic.status === "published") {
      ...
      setTopics((prev) =>
        prev.map((t) => (t.id === topic.id ? { ...t, status: "archived" } : t)),
      );
    } else {
  ...
  return (
    <div>
      ...
      <div css={S.meta}>
        {loading ? "Loading…" : `${filtered.length} of ${topics.length} topics`}
      </div>
          ...
          <tbody>
            {filtered.map((t) => (
              <tr key={t.id} css={S.tr}>
                ...
                <td css={S.td}>
                  <span css={S.badge(TOPIC_DIFF_COLOR[t.difficulty] ?? C.accent2)}>
                    {t.difficulty}
                  </span>
                </td>
                <td css={S.td}>
                ...
                <td css={S.td} style={{ color: C.muted }}>
                  {t.questionCount}
                </td>
                <td css={S.td}>
                  <div css={S.actions}>
                    ...
                    <button
                      css={S.iconBtn(
                        t.status === "published" ? C.muted : C.accent3,
                      )}
