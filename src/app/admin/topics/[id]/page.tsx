...
export default function EditTopicPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    getTopic(params.id).then(t => { setTopic(t); setLoading(false) })
  }, [params.id])
  ...
  if (loading) return <div style={{ display: 'flex', justifyContent: 'center', padding: '3rem' }}>Loading…</div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.75rem' }}>
        ...
        <h1 style={{ fontSize: '1.5rem', fontWeight: 900 }}>Edit: {topic.keyword}</h1>
      </div>
      <TopicForm mode="edit" initial={topic} onSubmit={handleSubmit} onDelete={handleDelete} />
    </div>
  )
}