...
import { BookmarkIcon, CheckCircle, ChevronDown, Lock } from 'lucide-react'
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="h-2 bg-surface rounded-full mt-3 overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-accent to-accent3 rounded-full transition-all duration-500"
              style={{ width: `${pct}%` }}
            />
          </div>
          <div className="flex justify-between mt-1">
            ...
            {!progress.isPro && (
              <span className="text-yellow-400/70 text-xs">
                {FREE_LIMIT - Math.min(masteredCount, FREE_LIMIT)} free marks remaining
              </span>
            )}
          </div>
        </div>
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {q.tags.map(t => (
                        <span key={t} className={clsx('text-[10px] font-bold px-2 py-0.5 rounded-md', tagStyle[t])}>
                          {tagLabel[t]}
                        </span>
                      ))}
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-accent/10 text-accent/80 border border-accent/20">
                        {q.cat}
                      </span>
                    </div>
                  </div>
                  <div className={clsx('flex-shrink-0 transition-transform duration-300', isOpen && 'rotate-180')}>
                ...
                {isOpen && (
                  <div className="border-t border-border">
                    ...
                    <div
                      className="answer px-5 py-4"
                      dangerouslySetInnerHTML={{ __html: q.answer }}
                    />
                    {/* Action buttons */}
                    <div className="flex gap-3 px-5 pb-5">
                      <button
                        onClick={() => handleMastered(q.id, !isMastered)}
                        className={clsx(
                          'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border',
                          isMastered
                            ? 'bg-accent3/20 border-accent3/40 text-accent3'
                            : 'border-border text-muted hover:border-accent3/50 hover:text-white'
                        )}
                      >
                        <CheckCircle size={13} />
                      ...
                      <button
                        onClick={() => handleBookmark(q.id, !isBookmarked)}
                        className={clsx(
                          'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border',
                          isBookmarked
                            ? 'bg-accent2/20 border-accent2/40 text-accent2'
                            : 'border-border text-muted hover:border-accent2/50 hover:text-white'
                        )}
                      >
