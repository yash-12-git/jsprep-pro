...
import { getWeeklyLeaderboard, type LeaderboardEntry } from '@/lib/userProgress'
...
const row = (rank: number, isCurrentUser: boolean) => css`
  ...
  border: 1px solid ${isCurrentUser ? 'rgba(124,106,247,0.3)' : rank <= 3 ? rankBorder(rank) : C.border};
...
const rankBadge = (rank: number) => css`
  ...
  font-weight: 800;
  background: ${rank === 1 ? 'rgba(247,199,106,0.15)' : rank === 2 ? 'rgba(192,192,192,0.1)' : rank === 3 ? 'rgba(205,127,50,0.1)' : 'rgba(255,255,255,0.04)'};
  color: ${rank === 1 ? '#f7c76a' : rank === 2 ? '#c0c0c0' : rank === 3 ? '#cd7f32' : 'rgba(255,255,255,0.3)'};
`
...
interface Props {
  currentUid?: string;
  setTab: (tab: 'home' | 'practice' | 'learn' | 'community') => void;
}

export default function Leaderboard({ currentUid , setTab}: Props) {
  ...
  return (
    <div css={wrapper}>
      ...
      {loading ? (
        <div css={list}>
          {[...Array(5)].map((_, i) => <div key={i} css={skeletonRow} />)}
        </div>
      ) : entries.length === 0 ? (
        <div css={emptyState}>
          <div css={emptyTitle}>Nobody on the board yet!</div>
          <div css={emptyDesc}>Master questions to earn XP and claim your spot.</div>
          <button onClick={()=> setTab('practice')} css={ctaLink}>Start earning XP →</button>
        </div>
      ) : (
        <div css={list}>
          {entries.map((entry, i) => {
            ...
            return (
              <div key={entry.uid} css={row(rank, isCurrentUser)}>
                ...
                <div css={xpBadge}>
                  <Zap size={11} />
                  {entry.weeklyXp}
                  <span style={{ fontSize: '0.625rem', color: 'rgba(255,255,255,0.35)', fontWeight: 600 }}>XP</span>
                </div>
              </div>
            )
          })}
      ...
      <div css={weekNote}>
        XP earned: 10 per mastered question · 8 per output/debug solve · 5 per quiz correct
      </div>
    </div>
  )
}