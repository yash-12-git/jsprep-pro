...
interface StudyPlan {
  ...
  quickWins: string[]; focusQuestions: number[]
}
...
export default function StudyPlanPage() {
  ...
  if (!progress.isPro) return <><Navbar /><PaywallBanner reason="AI Study Plan is a Pro feature. Upgrade to get a personalized roadmap!" /></>

  const catStats = CATEGORIES.map(cat => {
    const qs = questions.filter(q => q.cat === cat)
    const m = qs.filter(q => progress.masteredIds.includes(q.id)).length
    return { cat, pct: Math.round((m / qs.length) * 100) }
  })
  ...
  async function generatePlan() {
    ...
    try {
      ...
      const res = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'studyplan', messages: [{ role: 'user', content: 'Generate my study plan.' }],
          context: { masteredIds: progress.masteredIds, quizScores: progress.quizScores,
            weakCategories, strongCategories, interviewDate: interviewDate || null, streakDays: progress.streakDays } }) })
  ...
  return (
    <>
      ...
      <div css={Shared.pageWrapper}>
        ...
        {/* Setup card */}
        <div css={S.inputCard}>
          <p css={[S.inputLabel, { marginBottom: '0.5rem' }]}>When is your interview? <span css={{ textTransform: 'none', letterSpacing: 0, color: C.muted, fontWeight: 400 }}>(optional)</span></p>
          ...
          <div css={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', margin: '1.25rem 0' }}>
            {[
              { v: progress.masteredIds.length, l: 'Mastered', c: C.accent },
          ...
          <button css={Shared.primaryBtn(C.accent2)} onClick={generatePlan} disabled={generating}>
            {generating ? <><Loader2 size={15} css={{ animation: 'spin 1s linear infinite' }} /> Analyzing...</>
              : plan ? <><RefreshCw size={15} /> Regenerate Plan</> : <><Zap size={15} /> Generate My Study Plan</>}
          </button>
        </div>
        ...
        {plan && (
          <div css={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div css={[S.section, { textAlign: 'center' }]}>
              <div css={{ fontSize: '3.5rem', fontWeight: 900, color: readColor(plan.readinessScore), marginBottom: '0.25rem' }}>{plan.readinessScore}%</div>
              ...
              <div css={[S.section, { borderColor: `${C.accent}33` }]}>
                ...
                {focusQs.map(q => q && (
                  <button key={q.id} onClick={() => router.push('/dashboard')}
                    css={{ display: 'block', width: '100%', textAlign: 'left', padding: '0.75rem', background: C.surface, border: `1px solid ${C.border}`, borderRadius: '0.75rem', marginBottom: '0.5rem', cursor: 'pointer', transition: 'border-color 0.15s ease', '&:hover': { borderColor: `${C.accent}66` } }}>
                    <span css={{ fontFamily: 'monospace', fontSize: '0.75rem', color: C.accent, marginRight: '0.5rem' }}>Q{String(q.id + 1).padStart(2, '0')}</span>
                    <span css={{ fontSize: '0.75rem', color: 'white' }}>{q.q}</span>
                  </button>
                ))}
              </div>
            )}
